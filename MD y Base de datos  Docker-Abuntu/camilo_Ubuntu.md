 Docker Ubuntu con MySQL, PostgreSQL, MongoDB y SQL Server

--paso a paso cómo tomar una imagen Ubuntu y convertirla en un servidor con los 4 motores de bases de datos instalados y accesibles desde el exterior.
...
Descargar la versión más reciente de Ubuntu 22.04 desde Docker Hub.
...
Instalar herramientas necesarias para gestionar repositorios, puertos y servicios.
...
sql
ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY 'RootMySQL!234';
...
su - postgres
psql
CREATE USER camilo WITH PASSWORD 'camilo123!';
ALTER ROLE camilo WITH SUPERUSER;
\q
...
```
mongodb://camilo:camilo123!@localhost:27095/admin?authSource=admin
```
...
bash
sqlcmd -S localhost,1433 -U SA -P "Camilo123!" -C
